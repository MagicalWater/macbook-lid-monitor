...
| 13 | Enabled bounded acceptance — recovery resleep command ready | validation evidence | dry-run path and first enabled sleep complete; bounded recovery-resleep command implemented; injected failure remains | recovery resleep approved; injected failure remains non-sleeping | automatic disable/bootout/bootstrap cleanup |
